...
class Turret :
    ...
    def __del__(self) :
        for l in Turret.ctrl.launchers:
            l.stop_movement()
...
class RocketCommander :
    ...
    def interpret(self, text) :
        ...
        try :
            ...
            if e.errno == errno.EACCES or e.errno == errno.EAGAIN :     # EACCES is not EACCESS :-)
                result['code'].append(-1)
                result['msg'].append("alrady using. please wait few minuties.")        # alread locked
            else :
                raise IOError(e)        # exception to upper level
        else :
